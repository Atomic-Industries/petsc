...
/*TEST
  ...
  test:
    ...
    args: -run_type full -variable_coefficient nonlinear -petscspace_degree 1 \
          -dm_refine 2 -dm_coarsen_hierarchy 1 -dm_plex_location_alg grid_hash -dm_adaptor pragmatic \
  ...
  test:
    ...
    args: -run_type full -petscspace_degree 1 \
          -dm_refine 3 -dm_coarsen_hierarchy 3 -dm_plex_location_alg grid_hash -dm_adaptor pragmatic \
  ...
  testset:
    ...
    test:
      args: -pc_hpddm_coarse_mat_type baij -options_left no
      filter: grep -v "      total: nonzeros=" | grep -v "      rows=" | sed -e "s/total number of linear solver iterations=[1-2][5-7]/total number of linear solver iterations=16/g"
