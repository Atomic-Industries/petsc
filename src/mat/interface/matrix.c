...
.ve
    In Fortran the `MatGetRowIJ()` binding is deprecated (since PETSc 3.19), use `MatGetRowIJF90()`
...
.   done - `PETSC_TRUE` or `PETSC_FALSE` indicated that the values have been returned
    ...
    Fortran Note:
    In Fortran the `MatGetRowIJ()` binding is deprecated (since PETSc 3.19), use `MatRestoreRowIJF90()`
