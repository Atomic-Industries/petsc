...
PetscErrorCode MatAXPY_SeqDense(Mat Y, PetscScalar alpha, Mat X, MatStructure str)
{
  ...
  if (ldax > m || lday > m) {
    ...
    for (j = 0; j < X->cmap->n; j++) PetscCallBLAS("BLASaxpy", BLASaxpy_(&m, &alpha, xv + j * ldax, &one, yv + j * lday, &one));
  } else {
...
PetscErrorCode MatDuplicateNoCreate_SeqDense(Mat newi, Mat A, MatDuplicateOption cpvalues)
{
  ...
  if (cpvalues == MAT_COPY_VALUES) {
    ...
    m = A->rmap->n;
    if (lda > m || nlda > m) {
      for (j = 0; j < A->cmap->n; j++) PetscCall(PetscArraycpy(v + j * nlda, av + j * lda, m));
    } else {
...
.   array - pointer to the data
   ...
   Fortran Note:
   Use `MatDenseGetArrayF90()`, this routine is deprecated in Fortran (since version 3.19)
...
-  array - pointer to the data (may be NULL)
   ...
   Fortran Note:
   Use `MatDenseRestoreArrayF90()`, this routine is deprecated in Fortran (since version 3.19)
