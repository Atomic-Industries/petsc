...
class SectionBase(DocBase):
  ...
  def _check_duplicate_headers(self, docstring):
    ...
    """
    unique_header_diag = self.diags.section_header_unique
    for heading, where in self.seen_headers.items():
      ...
      docstring.add_error_from_diagnostic(Diagnostic(unique_header_diag, mess, self.extent.start))
    return
...
@DiagnosticManager.register(
  ...
  ("order","Verify that documentation for parameters is in order of appearance"),
  ("undocumented-parameter","Verify that all parameters for a symbol are documented"),
  ("extra-parameter","Verify that all parameters that are documented actually exist"),
