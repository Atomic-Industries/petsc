...
@DiagnosticManager.register(
  ('undocumented-parameter','Verify that all parameters for a symbol are documented'),
  ('extra-parameter','Verify that all documented parameters exist for a symbol'),
...
class FunctionParameterList(ParameterList):
  ...
  def _check_valid_param_list_from_cursor(self, linter, docstring, cursor_list):
    ...
    if cursor_list and not self:
      ...
      diag = docstring.make_diagnostic(
        self.diags.undocumented_parameter,
    ...
    if not cursor_list:
      ...
      if self and len(self.items.values()):
        ...
        docstring.add_error_from_source_range(
          self.diags.extra_parameter, mess, self.extent, highlight=False, patch=Patch(self.extent, '')
        )
      return
    ...
    if not_found:
      diag         = self.diags.extra_parameter
    ...
    undoc_param_diag = self.diags.undocumented_parameter
...
class Level(InlineList):
  ...
  def _check_valid_level_spelling(self, docstring):
    ...
    """
    casefold_diag = self.diags.casefold
    spelling_diag = self.diags.spelling
