...
class Linter:
  ...
  def _check_duplicate_function_calls(self, processed_funcs):
    dup_diag = self._check_duplicate_function_calls.diags.duplicate_function
    for pname, function_list in processed_funcs.items():
      ...
      for func, scope in function_list:
        ...
        if combo not in seen:
          ...
          seen_start = seen[combo][0].extent.start.line
          ...
          previous   = seen[combo][0].formatted(nbefore=2, nafter=startline - seen_start)
  ...
  def add_error_from_cursor(self, cursor, diagnostic):
    ...
    if cursor_id not in errors[filename]:
      header = f'\nERROR {len(errors[filename])}: {str(cursor)}\n'
      errors[filename][cursor_id] = WeakList([header, [], [], []])

    patch            = diagnostic.patch
    have_patch       = patch is not None
    cursor_id_errors = errors[filename][cursor_id]
